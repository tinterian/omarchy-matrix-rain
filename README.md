...
A full [Omarchy](https://omarchy.org) theme built around a Matrix-style
digital rain wallpaper: an animated, seamlessly-looping live wallpaper (four
selectable text sizes), plus the file-manager icons, Neovim/VS Code colors,
and lock screen that make it a complete theme rather than just a background.

<video src="https://raw.githubusercontent.com/tinterian/omarchy-matrix-rain/master/assets/preview.mp4"
       poster="https://raw.githubusercontent.com/tinterian/omarchy-matrix-rain/master/assets/preview-medium.jpg"
       controls loop muted playsinline width="100%"></video>

*(GitHub doesn't autoplay embedded video — click to play. The loop above is
the exact same mathematically-seamless loop the live wallpaper uses, just
recompressed smaller for this preview.)*

![theme switcher preview](theme/preview.png)
...
A theme, not just a wallpaper generator. Everything ships together and is
applied automatically the moment you select **Matrix** from Omarchy's theme
menu:

- The animated digital-rain **live wallpaper**, in four text sizes you flip
  through exactly like picture wallpapers on any other theme (see below).
- **File-manager icons** — a generated Matrix-recolored variant of the
  `beautyline` outline icon pack.
- **Neovim and VS Code color schemes** wired in through Omarchy's normal
  per-app templating.
- **Lock screen** wordmark recolored to match.
- A **theme-switcher preview** (`theme/preview.png`) that looks like an
  actual themed desktop, not a placeholder — see "Theme components" below
  for how each of these was built.

The wallpaper itself is two things layered together:
...
This renders all 4 wallpaper variants locally at your screen's resolution
(small, medium, large, xlarge text) — **expect roughly 15-25 minutes**, one
time. It installs:

- `~/.config/omarchy/themes/matrix/{colors.toml,icons.theme,neovim.lua,
  vscode.json,unlock.png,preview.png}` (each installed only if that
  specific file isn't already there — never overwrites an existing
  customization)
...
- **`preview.png`** — the thumbnail Omarchy's theme switcher (the image-grid
  menu you open to pick a theme) shows for Matrix: a real desktop screenshot
  (Neovim+Neo-tree with the Matrix colorscheme, a terminal, btop, and
  Nautilus with the BeautyLine-Matrix icons — the same fixed scene every
  stock theme's own preview.png shows, just recolored), cropped to
  1800x1012 (the same dimensions every stock theme's preview uses) and
  palette-quantized to match their file size. It's a fixed, curated asset —
  see `generate_matrix_preview.py`'s header for the full recipe if you ever
  need to regenerate it. `omarchy-theme-switcher` looks for this specific
  file — without it, the switcher falls back to showing the theme's first
  background file instead, which still works but isn't a curated thumbnail.
...
If you change `colors.toml` enough that `theme/preview.png` no longer
matches, see `generate_matrix_preview.py`'s header comment for how it was
built and how to regenerate it.
...
Removes the service, launcher, and generated wallpaper variants. Leaves the
rest of the theme (`colors.toml`, `icons.theme`, `neovim.lua`, `vscode.json`,
`unlock.png`, `preview.png`) in place — delete
`~/.config/omarchy/themes/matrix` yourself if you want the theme gone
entirely, or `~/.local/share/icons/BeautyLine-Matrix` too if you installed
the icon pack.
